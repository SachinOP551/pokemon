...
DEFAULT_LOCKED = ["Premium", "Limited Edition", "Ultimate", "Supreme"]
...
async def propose_callback(client: Client, callback_query: CallbackQuery):
    ...
    if action == "propose":
        if args[0] == "toggle":
            ...
            settings = await db.get_propose_settings()
            locked_rarities = settings.get('locked_rarities', DEFAULT_LOCKED)
            ...
            await db.update_propose_settings({'locked_rarities': locked_rarities})
