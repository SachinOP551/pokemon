...
def format_pokemon_type(pokemon_type):
    ...
    if "/" in pokemon_type:
        ...
        for type_name in types:
            ...
            formatted_types.append(f"{emoji} <b><i>{type_name.upper()}</i></b>")
        return " & ".join(formatted_types)
    else:
        # Single type
        emoji = TYPE_EMOJIS.get(pokemon_type.strip(), "❓")
        return f"{emoji} <b><i>{pokemon_type.strip().upper()}</i></b>"
...
def create_inline_result(character):
    ...
    caption = (
        f"<b>👤 Name: <i>{name}</i></b>\n"
        f"{rarity_emoji}<b>Rarity: <i>{rarity}</i></b>\n"
        f"⛩<b>Region: <i>{anime}</i></b>\n\n"
...
async def search_command(client: Client, message: Message):
    """Show search button for inline Pokémon search"""
    keyboard = [
        [InlineKeyboardButton("🔍 Search Pokémon", switch_inline_query_current_chat="")]
    ]
