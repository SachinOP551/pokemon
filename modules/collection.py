    ...
    if hasattr(db, 'pool'):  # PostgreSQL
        async with db.pool.acquire() as conn:
            ...
            characters = await conn.fetch(
                "SELECT character_id, name, rarity, anime, img_url, file_id, is_video FROM characters WHERE character_id = ANY($1)",
                char_ids_tuple
            )
            ...
            if hasattr(db, 'pool'):  # PostgreSQL
                async with db.pool.acquire() as conn:
                    char_docs = await conn.fetch(
                        "SELECT character_id, name, rarity, anime, file_id, img_url, is_video FROM characters WHERE character_id = $1",
                        favorite_id
                    )
...
async def smode_command(client, message):
    ...
    keyboard = [
        [InlineKeyboardButton("Sort by Anime 🧩", callback_data=f"sm_anime:{owner_id}"), InlineKeyboardButton("Sort by Rarity 📊", callback_data=f"sm_rarity:{owner_id}")],
...
async def handle_smode_callback(client, callback_query: CallbackQuery):
    ...
    if cmd == "sm_close":
        ...
        keyboard = [
            [InlineKeyboardButton("Sort by Anime 🧩", callback_data=f"sm_anime{owner_suffix}"), InlineKeyboardButton("Sort by Rarity 📊", callback_data=f"sm_rarity{owner_suffix}")],
...
def create_inline_result(character, user_name):
    ...
    caption += f"🎥<b>Anime: {character['anime']}</b>\n\n"
