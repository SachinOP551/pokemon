...
async def favorite_command(client: Client, message: Message):
    ...
    if len(args) < 2:
        await message.reply_text(
            "<b>❌ ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴛʜᴇ ᴘᴏᴋᴇᴍᴏɴ ɪᴅ ᴛᴏ sᴇᴛ ᴀs ғᴀᴠᴏʀɪᴛᴇ!</b>"
        )
        return
    ...
    try:
        ...
        await message.reply_text(
            "<b>❌ ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴀ ᴠᴀʟɪᴅ ɴᴜᴍʙᴇʀ ғᴏʀ ᴘᴏᴋᴇᴍᴏɴ ɪᴅ!</b>"
        )
        return
    ...
    try:
        ...
        if not character:
            await message.reply_text(
                "<b>❌ ᴘᴏᴋᴇᴍᴏɴ ɴᴏᴛ ғᴏᴜɴᴅ!</b>"
            )
            return
        
        if character_id not in user_data.get('characters', []):
            await message.reply_text(
                "<b>❌ ʏᴏᴜ ᴅᴏɴ'ᴛ ᴏᴡɴ ᴛʜɪs ᴘᴏᴋᴇᴍᴏɴ!</b>"
            )
            return
...
async def handle_favorite_callback(client: Client, callback_query: CallbackQuery):
    ...
    if callback_query.data == "fav_confirm":
        ...
        await callback_query.edit_message_caption(
            f"<b>✨ {character['name']} "
            "ʜᴀs ʙᴇᴇɴ sᴇᴛ ᴀs ʏᴏᴜʀ ғᴀᴠᴏʀɪᴛᴇ ᴘᴏᴋᴇᴍᴏɴ! 🎉</b>"
        )
        del _temp_data[user_id]
