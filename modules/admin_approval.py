...
# Example: OWNER_ID can be int or list[int]
OWNER_ID: Union[int, List[int]] = [6055447708, 6919874630, 7546387669]

async def create_approval_request(client: "Client", action: "AdminAction") -> bool:
    """Create an approval request and send it to the owner(s)."""
    try:
        # Store the action
        ...
        display_name = action_display_names.get(
            action.action_type, action.action_type.title()
        )
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ Approve", callback_data=f"admin_approve_{str(action.unique_id)}"
                ),
                InlineKeyboardButton(
                    "❌ Decline", callback_data=f"admin_decline_{str(action.unique_id)}"
                ),
            ]
        ]
        ...
        # Send approval request to one or multiple owners
        owner_ids = OWNER_ID if isinstance(OWNER_ID, list) else [OWNER_ID]
        for owner in owner_ids:
            await client.send_message(int(owner), approval_text, reply_markup=reply_markup)
...
async def execute_approved_action(client: Client, action: AdminAction) -> bool:
    ...
    try:
        ...
        if action.action_type == 'give':
            ...
            char_ids = action.details.get('char_ids', [])
            # Add each character individually using the existing method
            ...
            if not target_data:
                # Add user with basic fields that the PostgreSQL add_user method supports
                await db.add_user({
...
async def log_approved_action(client: Client, action: AdminAction, success: bool = True):
    ...
    try:
        ...
        # Send specific logs for token actions
            ...
            await send_token_log(client, admin_user, target_user, amount, action_name)
