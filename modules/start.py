...
# Lightweight in-memory cache for character details
CHAR_DETAILS_CACHE = {}
CHAR_DETAILS_TTL_SECONDS = 60
CHAR_DETAILS_MAX_SIZE = 200

def _prune_character_cache_if_needed():
    if len(CHAR_DETAILS_CACHE) <= CHAR_DETAILS_MAX_SIZE:
        return
    # Remove oldest 10% entries
    to_remove = max(1, CHAR_DETAILS_MAX_SIZE // 10)
    sorted_items = sorted(CHAR_DETAILS_CACHE.items(), key=lambda kv: kv[1]['ts'])
    for key, _ in sorted_items[:to_remove]:
        CHAR_DETAILS_CACHE.pop(key, None)

async def get_character_cached(db, character_id: int):
    """Return character details from cache or DB with short TTL."""
    now_ts = datetime.now().timestamp()
    entry = CHAR_DETAILS_CACHE.get(character_id)
    if entry and (now_ts - entry['ts'] < CHAR_DETAILS_TTL_SECONDS):
        return entry['data']
    character = await db.get_character(character_id)
    if character:
        CHAR_DETAILS_CACHE[character_id] = {'data': character, 'ts': now_ts}
        _prune_character_cache_if_needed()
    return character

async def start_command(client: Client, message: Message):
    print("[DEBUG] start_command handler called")
    print(f"Start command received from {message.from_user.id}")
    # Per-user lightweight rate limit for /start (including deep-links)
    # Allow 2 requests per 5 seconds per user
    if not hasattr(start_command, "_rate_limiter"):
        start_command._rate_limiter = RateLimiter(max_requests=2, time_window=5)
    user_id_for_rl = message.from_user.id if message.from_user else None
    if user_id_for_rl is not None:
        allowed = await start_command._rate_limiter.acquire(user_id_for_rl)
        if not allowed:
            wait_left = int(start_command._rate_limiter.get_wait_time(user_id_for_rl)) + 1
            return
    ...
    if message.text and message.text.startswith('/start details_'):
        try:
            ...
            character = await get_character_cached(db, character_id)
            if not character:
            ...
            # Don't block normal /start if details flow fails for any reason
            print(f"[ERROR] Error handling details request (falling back to normal start): {e}")
