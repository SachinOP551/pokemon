...
class TeamManager:
    ...
    async def get_team_details(user_id: int) -> Optional[Dict]:
        ...
        try:
            # Get Pokemon details from characters table
            pool = get_postgres_pool()
            async with pool.acquire() as conn:
...
async def team_command(client: Client, message: Message):
    ...
    if not team_details:
        # No team exists, show creation options
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ Create Team", callback_data="create_team")],
            [InlineKeyboardButton("📖 How to use teams", callback_data="team_help")]
        ])
        
        await message.reply_text(
            "<b>🏆 Pokemon Team Manager</b>\n\n"
            "Welcome to the team system! 🎉\n\n"
            "You don't have a team yet. Create your first team to organize up to 3 of your favorite Pokemon.\n\n"
            "<b>What teams can do:</b>\n"
            "• 🎯 Organize your strongest Pokemon\n"
            "• ⚡ Quick access to your favorites\n"
            "• 🥊 Battle with your selected team\n"
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("➕ Add Pokemon", callback_data="add_to_team"),
            InlineKeyboardButton("➖ Remove Pokemon", callback_data="remove_from_team")
        ],
        [
            InlineKeyboardButton("✏️ Rename Team", callback_data="rename_team"),
            InlineKeyboardButton("🔄 Clear Team", callback_data="clear_team")
        ],
        [InlineKeyboardButton("❓ Help", callback_data="team_help")]
    ])
    
    await message.reply_text(team_text, reply_markup=keyboard)
...
async def team_callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    if data == "create_team":
        await callback_query.message.edit_text(
            ...
            "You can add up to 3 Pokemon to your team.\n"
            "Use <code>/mycollection</code> to see your Pokemon and their IDs.",
