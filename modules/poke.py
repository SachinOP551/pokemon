...
from modules.battle import RARITY_MOVES, RARITY_STATS
...
POKEMON_NAME_MAP = {
    ...
    "mega shiny gardevoir": "gardevoir-mega",
...
async def _fetch_pokeapi_moves(pokemon_name: str, session: aiohttp.ClientSession) -> Optional[List[dict]]:
    """Fetch top damaging moves for a Pokémon, ensuring moves of all its types are included."""
    ...
    try:
        ...
        # Pokémon types
        pokemon_types = [t["type"]["name"].title() for t in data.get("types", [])]

        best: List[Tuple[str, str, int, int, float, str]] = []
        ...
        # Ensure at least one move per Pokémon type
        type_moves = {}
        for t in pokemon_types:
            for move in best_sorted:
                if move[1] == t:
                    type_moves[t] = {"name": move[0], "type": move[1], "power": move[2], "accuracy": move[3], "class": move[5]}
                    break

        # Start result with type moves
        result = list(type_moves.values())
        used_types = set(move["type"] for move in result)
        has_normal = any(move["type"] == "Normal" for move in result)

        # Fill remaining slots with top-scoring moves
...
async def _moves_text(character: dict) -> str:
    """Get comprehensive moveset text with PokeAPI + fallbacks"""
        ...
        try:
            ...
            if isinstance(char_moves, str):
                ...
                try:
                    ...
                    # Not JSON, attempt to parse free-form string
                    ...
                    for chunk in chunks:
                        ...
                        m1 = re.match(
                            r"^(.*?)\s*\(\s*Type:\s*([A-Za-z ]+)\s*,\s*Power:\s*(\d+)\s*,\s*Acc:\s*(\d+)%\s*\)\s*$",
                            chunk,
                        )
                        # Pattern without Type
                        m2 = re.match(
                            r"^(.*?)\s*\(\s*Power:\s*(\d+)\s*,\s*Acc:\s*(\d+)%\s*\)\s*$",
                            chunk,
                        )
                        if m1:
