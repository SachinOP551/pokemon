...
class PostgresDatabase:
    ...
    async def get_weekly_top_collectors(self, limit: int = 10):
        """Get this week's top collectors efficiently using JSONB queries (Monday to Sunday)."""
        try:
            from datetime import datetime, timedelta
            
            # Calculate the start of the current week (Monday) and end (next Monday)
            utc_now = datetime.utcnow()
            ...
            async with self.pool.acquire() as conn:
                rows = await conn.fetch(query, week_start, week_end, limit)
            ...
            logger.error(f"Error getting weekly top collectors: {e}")
            return []
