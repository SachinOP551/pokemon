    ...
    async def create_challenge(self, challenger_id: int, opponent_id: int, chat_id: int) -> Optional[str]:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                # Enforce daily overall limit of 25 battles per user (last 24 hours)
                try:
                    challenger_daily_row = await conn.fetchrow(
                        '''
                        SELECT COUNT(*) AS cnt
                        FROM battles
                    ...
                    opponent_daily_row = await conn.fetchrow(
                        '''
                        SELECT COUNT(*) AS cnt
                        FROM battles
                        WHERE (challenger_id = $1 OR opponent_id = $1)
                    ...
                    challenger_count = int(challenger_daily_row['cnt']) if challenger_daily_row and 'cnt' in challenger_daily_row else 0
                    opponent_count = int(opponent_daily_row['cnt']) if opponent_daily_row and 'cnt' in opponent_daily_row else 0
                    if challenger_count >= 25:
                        return "challenger_daily_overall_limit"
                    if opponent_count >= 25:
                        return "opponent_daily_overall_limit"
                except Exception as e:
                    # If counting fails, do not block the challenge; just log
                    print(f"Warning: failed to check daily overall battle limit: {e}")
        except Exception as e:
    ...
    async def accept_challenge(self, opponent_id: int) -> Optional[Battle]:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                # Enforce daily overall limit of 25 battles per user (last 24 hours) for both users
                try:
                    challenger_daily_row = await conn.fetchrow(
                        '''
                        SELECT COUNT(*) AS cnt
                    ...
                    challenger_count = int(challenger_daily_row['cnt']) if challenger_daily_row and 'cnt' in challenger_daily_row else 0
                    opponent_count = int(opponent_daily_row['cnt']) if opponent_daily_row and 'cnt' in opponent_daily_row else 0
                    if challenger_count >= 25 or opponent_count >= 25:
                        return None
                except Exception as e:
                    print(f"Warning: failed to check daily overall battle limit on accept: {e}")
        except Exception as e:
...
async def battle_command(client: Client, message: Message):
    ...
    elif result == "challenger_daily_overall_limit":
        await message.reply_text(
            "⛔ <b>Daily Limit Reached</b>\n\n"
            "You have reached your daily limit of <b>25 battles</b>.\n"
            "Please try again after 24 hours."
        )
    elif result == "opponent_daily_overall_limit":
        await message.reply_text(
            "⛔ <b>Opponent Limit Reached</b>\n\n"
            "Your opponent has reached their daily limit of <b>25 battles</b>.\n"
            "Please try again later or choose another opponent."
        )
