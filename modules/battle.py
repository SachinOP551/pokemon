...
class Battle:
    ...
    def __init__(self, challenger_id: int, opponent_id: int, chat_id: int):
        ...
        # Turn timeout tracking (5 minutes)
        self.turn_timeout_task = None
        self.last_action_time = datetime.now()
        self.timeout_warnings_sent = set()  # Track which users have been warned
    ...
    async def _get_pokemon_details(self, pokemon_ids: List[int]) -> List[Dict]:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                # Convert to dicts and preserve the original team order from pokemon_ids
                unsorted = [dict(row) for row in rows]
                order_index = {pid: idx for idx, pid in enumerate(pokemon_ids)}
                result = sorted(
                    unsorted,
                    key=lambda p: order_index.get(p.get('character_id'), len(order_index) + 999)
                )
                # Enrich each pokemon with PokeAPI stats and moves (optimized with concurrent requests)
                await self._enrich_pokemon_concurrent(result)
    ...
    async def _enrich_pokemon_concurrent(self, pokemon_list: List[Dict]):
        """Concurrently enrich Pokemon with API data for faster loading"""
        import time
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60)) as session:
                # Create concurrent tasks for all Pokemon
                tasks = []
                for p in pokemon_list:
                    name = (p.get('name') or '').strip()
                    if name:
                        tasks.append(self._enrich_single_pokemon(p, name, session, fetch_pokeapi_stats, _fetch_pokeapi_moves, _get_necrozma_form_data))
                
                # Execute all API calls concurrently with a reasonable timeout
                if tasks:
                    print(f"Starting concurrent enrichment for {len(tasks)} Pokemon...")
                    await asyncio.gather(*tasks, return_exceptions=True)
                    
            elapsed = time.time() - start_time
            print(f"Pokemon enrichment completed in {elapsed:.3f}s (concurrent mode)")
            
        except Exception as e:
            print(f"Error in concurrent Pokemon enrichment: {e}")
    
    async def _enrich_single_pokemon(self, pokemon_data: Dict, name: str, session, fetch_pokeapi_stats, _fetch_pokeapi_moves, _get_necrozma_form_data):
        """Enrich a single Pokemon with API data"""
        try:
            # First, try local Necrozma JSON for forms (fast)
            necro = _get_necrozma_form_data(name)
            if necro:
                # Stats from JSON
                s = (necro.get('max_level_stats') or {})
                if s:
                    pokemon_data['api_stats'] = {
                        'hp': s.get('hp', 0),
                        'atk': s.get('atk', 0),
                        'def': s.get('def', 0),
                ...
                # Types if DB empty
                if not (pokemon_data.get('type') or '').strip():
                    types = necro.get('types') or []
                    if types:
                        pokemon_data['type'] = "/".join(types)
                # Moves from JSON take priority
                moves = necro.get('moves') or []
                if moves:
                    pokemon_data['api_moves'] = moves
                    return  # Skip PokeAPI if local data exists
            
            # Concurrently fetch both stats and moves from PokeAPI
            stats_task = asyncio.create_task(fetch_pokeapi_stats(name))
            moves_task = asyncio.create_task(_fetch_pokeapi_moves(name, session))
            
            # Wait for both with timeout
            api_stats, api_moves = await asyncio.gather(
                asyncio.wait_for(stats_task, timeout=10.0),  # 10 second timeout per Pokemon
                asyncio.wait_for(moves_task, timeout=10.0),
                return_exceptions=True
            )
            
            # Process stats if successful
            if not isinstance(api_stats, Exception) and api_stats and api_stats.get('max_level_stats'):
            ...
            # Process moves if successful
            if not isinstance(api_moves, Exception) and api_moves:
                pokemon_data['api_moves'] = api_moves
                
            print(f"✓ Enriched: {name}")
                
        except asyncio.TimeoutError:
            print(f"⚠ Timeout enriching {name}")
        except Exception as e:
...
class BattleManager:
    ...
    async def _start_turn_timeout(self, battle: Battle, client: Client):
        """Start a 5-minute timeout for the current turn"""
        # Cancel any existing timeout
        await self._cancel_turn_timeout(battle)
        
        # Create new timeout task
        battle.turn_timeout_task = asyncio.create_task(
            self._handle_turn_timeout(battle, client)
        )
        print(f"Started turn timeout for battle {battle.battle_id}, user {battle.current_turn_user_id}")
    
    async def _cancel_turn_timeout(self, battle: Battle):
        """Cancel the current turn timeout"""
        if battle.turn_timeout_task and not battle.turn_timeout_task.done():
            battle.turn_timeout_task.cancel()
            try:
                await battle.turn_timeout_task
            except asyncio.CancelledError:
                pass
            print(f"Cancelled turn timeout for battle {battle.battle_id}")
        battle.turn_timeout_task = None
    
    async def _handle_turn_timeout(self, battle: Battle, client: Client):
        """Handle turn timeout - forfeit the battle for the inactive user"""
        try:
            # Wait for 4 minutes before sending warning
            await asyncio.sleep(240)  # 4 minutes
            
            # Check if battle is still active and user hasn't taken action
            if (battle.status not in ["active", "waiting_for_switch"] or 
                battle.battle_id not in self.active_battles):
                return
            
            # Send warning message
            inactive_user_id = battle.current_turn_user_id
            if battle.status == "waiting_for_switch":
                inactive_user_id = battle.waiting_for_switch_user_id
            
            if inactive_user_id not in battle.timeout_warnings_sent:
                try:
                    inactive_user = await client.get_users(inactive_user_id)
                    inactive_name = inactive_user.first_name or "Unknown"
                    
                    # Update battle UI with warning
                    if battle.status == "active":
                        battle_text, keyboard = await battle.get_battle_ui(battle.current_turn_user_id, client)
                    else:
                        battle_text, keyboard = await battle.get_switch_pokemon_ui(inactive_user_id, client)
                    
                    warning_text = (
                        f"⚠️ <b>WARNING:</b> <a href='tg://user?id={inactive_user_id}'>{inactive_name}</a> has 60 seconds to take action or the battle will be forfeited!\n\n"
                    )
                    ...
                    async with battle.ui_lock:
                        await client.edit_message_text(
                            ...
                            warning_text + battle_text,
                            reply_markup=keyboard
                        )
                    
                    battle.timeout_warnings_sent.add(inactive_user_id)
                    print(f"Sent timeout warning for battle {battle.battle_id}, user {inactive_user_id}")
                except Exception as e:
                    print(f"Error sending timeout warning: {e}")
            
            # Wait for final minute
            await asyncio.sleep(60)  # 1 minute
            
            # Check again if battle is still active and user hasn't taken action
            if (battle.status not in ["active", "waiting_for_switch"] or 
                battle.battle_id not in self.active_battles):
                return
            
            # Time is up - forfeit the battle
            await self._forfeit_battle(battle, inactive_user_id, client)
            
        except asyncio.CancelledError:
            print(f"Turn timeout cancelled for battle {battle.battle_id}")
        except Exception as e:
            print(f"Error in turn timeout handler: {e}")
            import traceback
            traceback.print_exc()
    
    async def _forfeit_battle(self, battle: Battle, forfeiting_user_id: int, client: Client):
        """Forfeit the battle due to inactivity - no winner declared, only penalty for forfeiting user"""
        try:
            print(f"Forfeiting battle {battle.battle_id} for inactive user {forfeiting_user_id}")
            
            # Determine other user (not the forfeiting one)
            if forfeiting_user_id == battle.challenger_id:
                other_user_id = battle.opponent_id
            else:
                other_user_id = battle.challenger_id
            
            # Set battle as finished with NO winner (forfeit scenario)
            battle.status = "finished"
            battle.winner_id = None  # No winner in forfeit scenario
            battle.finished_at = datetime.now()
            
            # Get user names for display
            try:
                other_user = await client.get_users(other_user_id)
                other_user_name = other_user.first_name or "Unknown"
            except:
                other_user_name = f"User {other_user_id}"
            
            try:
                forfeiting_user = await client.get_users(forfeiting_user_id)
                forfeiting_user_name = forfeiting_user.first_name or "Unknown"
            except:
                forfeiting_user_name = f"User {forfeiting_user_id}"
            
            # Process forfeit penalties (only forfeiting user loses tokens)
            forfeit_penalty_tokens = 10000
            
            try:
                # Only deduct tokens from the forfeiting user
                await self._deduct_tokens(forfeiting_user_id, forfeit_penalty_tokens)
            except Exception as e:
                print(f"Error processing forfeit penalties: {e}")
            
            # Create forfeit result text - no winner declared
            forfeit_text = (
                f"⏰ <b>Battle Forfeited</b> ⏰\n\n"
                f"<a href='tg://user?id={forfeiting_user_id}'>{forfeiting_user_name}</a> was inactive for 5 minutes and has forfeited the battle!\n\n"
                f"<b>Result:</b> No winner (forfeit) 🚫\n"
                f"<b>Forfeited by:</b> <a href='tg://user?id={forfeiting_user_id}'>{forfeiting_user_name}</a> ⏰\n\n"
                f"💰 <b>Forfeit Penalty:</b>\n"
                f"• <a href='tg://user?id={other_user_id}'>{other_user_name}</a> gains: No reward (forfeit)\n"
                f"• <a href='tg://user?id={forfeiting_user_id}'>{forfeiting_user_name}</a> loses: -{forfeit_penalty_tokens:,} tokens"
            )
            ...
            if battle.battle_id in self.active_battles:
                ...
                print(f"Battle {battle.battle_id} removed from active battles due to forfeit")
            ...
            print(f"Battle {battle.battle_id} successfully forfeited by user {forfeiting_user_id}")
            
        except Exception as e:
            print(f"Error forfeiting battle: {e}")
            import traceback
            traceback.print_exc()
    
    def _reset_turn_timer(self, battle: Battle):
        """Reset the turn timer when a user takes action"""
        battle.last_action_time = datetime.now()
        battle.timeout_warnings_sent.clear()
        print(f"Turn timer reset for battle {battle.battle_id}")
    ...
    async def create_challenge(self, challenger_id: int, opponent_id: int, chat_id: int) -> Optional[str]:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                # Enforce daily per-opponent limit of 25 battles (last 24 hours)
                try:
                    challenger_vs_opponent_row = await conn.fetchrow(
                        '''
                        SELECT COUNT(*) AS cnt
                        FROM battles
                        WHERE (
                    ...
                    battles_between_users = int(challenger_vs_opponent_row['cnt']) if challenger_vs_opponent_row and 'cnt' in challenger_vs_opponent_row else 0
                    if battles_between_users >= 25:
                        return "daily_per_opponent_limit"
                except Exception as e:
                    # If counting fails, do not block the challenge; just log
                    print(f"Warning: failed to check daily per-opponent battle limit: {e}")
        except Exception as e:
    ...
    async def accept_challenge(self, opponent_id: int) -> Optional[Battle]:
        ...
        print(f"🚀 Starting battle acceptance process for challenger {challenger_id} vs opponent {opponent_id}")
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                try:
                    ...
                    print(f"Warning: failed to check daily per-opponent battle limit on accept: {e}")
        except Exception as e:
            print(f"Error checking token requirements during accept: {e}")
            return None
        
        # Create battle
        battle_create_time = time.time()
        battle = Battle(challenger_id, opponent_id, 0)  # chat_id will be set when battle starts
        
        print(f"⏱️  Initializing teams...")
        team_init_start = time.time()
        await battle.initialize_teams()
        team_init_time = time.time() - team_init_start
        print(f"✅ Team initialization completed in {team_init_time:.3f}s")
        
        if battle.can_start_battle():
            battle.start_battle()
            self.active_battles[battle.battle_id] = battle
            
            total_time = time.time() - accept_start_time
            print(f"🏁 Battle acceptance completed in {total_time:.3f}s (team init: {team_init_time:.3f}s)")
            return battle
        
        print(f"❌ Battle cannot start - insufficient teams")
        return None
    ...
    async def start_interactive_battle(self, battle: Battle, client: Client, chat_id: int) -> bool:
        ...
        try:
            ...
            battle_text, keyboard = await battle.get_battle_ui(battle.current_turn_user_id, client)
            ui_generation_time = time.time() - ui_start_time
            print(f"🎨 UI generation completed in {ui_generation_time:.3f}s")
            ...
            await self._start_turn_timeout(battle, client)
        ...
        try:
            ...
            if result.get('fainted', False):
                ...
                if alive_pokemon_count == 0:
                    ...
                    # Reset timer and start timeout for switching
                    self._reset_turn_timer(battle)
                    await self._start_turn_timeout(battle, client)
            ...
            # Reset turn timer and start timeout for next turn
            self._reset_turn_timer(battle)
            await self._start_turn_timeout(battle, client)
    ...
    async def show_battle_results(self, battle: Battle, client: Client):
        ...
        try:
            ...
            if winner_id:
                ...
                # Process rewards (gated by defeated user's team size)
                # Determine loser team size to prevent farming against tiny teams
                if winner_id == battle.challenger_id:
                    loser_team_size = len(battle.opponent_team or [])
                else:
                    loser_team_size = len(battle.challenger_team or [])

                base_winner_tokens = 85000
                loser_penalty_tokens = 8000

                # Only award winner tokens if the defeated user's team had 5+ Pokemon
                if loser_team_size >= 5:
                    winner_reward_tokens = base_winner_tokens
                else:
                    winner_reward_tokens = 0

                # Only tokens (no shards) are awarded now
                try:
                    await self._give_rewards(winner_id, winner_reward_tokens, 0)
                ...
                # Build rewards body; hide shards message if none awarded due to cap
                rewards_lines = [
                    f"<b>Winner:</b> <a href='tg://user?id={winner_id}'>{winner_name}</a> 🎉",
                    f"<b>Loser:</b> <a href='tg://user?id={loser_id}'>{loser_name}</a> 😔",
                    f"💰 <b>Rewards:</b>",
                ]
                if winner_reward_tokens > 0:
                    rewards_lines.append(
                        f"• <a href='tg://user?id={winner_id}'>{winner_name}</a> earned: +{winner_reward_tokens:,} tokens"
                    )
                rewards_lines.append(
    ...
    async def execute_pokemon_switch(self, battle: Battle, pokemon_index: int, user_id: int, client: Client):
        ...
        try:
            ...
            # Reset turn timer and start timeout for the new turn
            self._reset_turn_timer(battle)
            await self._start_turn_timeout(battle, client)
...
async def battle_command(client: Client, message: Message):
    ...
    if result == "challenge_created":
        ...
        challenge_text = (
            ...
            f"🎯 <b>Battle Format:</b> 6 vs 6 Pokemon\n"
            f"⚡️ <b>Battle Type:</b> Turn-based PvP\n"
            f"💰 <b>Rewards:</b> Winner gets 85K tokens, Loser loses 8K tokens\n"
    ...
    elif result == "daily_per_opponent_limit":
        await message.reply_text(
            "⛔ <b>Daily Battle Limit Reached</b>\n\n"
            f"You and <a href='tg://user?id={opponent_id}'>{opponent_name}</a> have already battled <b>25 times</b> today.\n"
            "Please wait 24 hours or challenge a different opponent."
        )
...
async def battleinfo_command(client: Client, message: Message):
    ...
    help_text = (
        ...
        "• 🔢 <b>25 battles max</b> per opponent per day\n\n"
        "<b>Battle Rewards:</b>\n"
        "• 🏆 <b>Winner:</b> +85,000 tokens\n"
...
async def battle_callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    if data.startswith("accept_battle_"):
        ...
        if battle.challenger_id in battle.run_votes and battle.opponent_id in battle.run_votes:
            ...
            # Cancel any remaining timeout
            await battle_manager._cancel_turn_timeout(battle)
        ...
        # Reset timer and start timeout for the resumed battle
        battle_manager._reset_turn_timer(battle)
        await battle_manager._start_turn_timeout(battle, client)
        # Show battle UI
        battle_text, keyboard = await battle.get_battle_ui(battle.current_turn_user_id, client)
        async with battle.ui_lock:
            await client.edit_message_text(
