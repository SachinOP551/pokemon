    ...
    async def create_challenge(self, challenger_id: int, opponent_id: int, chat_id: int) -> Optional[str]:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                # Enforce daily limit of battles between the same pair (last 24 hours)
                try:
                    battle_count_row = await conn.fetchrow(
                        '''
                        SELECT COUNT(*) AS cnt
                        FROM battles
                    ...
                    pair_count = int(battle_count_row['cnt']) if battle_count_row and 'cnt' in battle_count_row else 0
                    if pair_count >= 15:
                        return "daily_limit_reached"
                except Exception as e:
                    # If counting fails, do not block the challenge; just log
                    print(f"Warning: failed to check daily battle limit: {e}")
        except Exception as e:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                try:
                    ...
                    print(f"Warning: failed to check daily battle limit on accept: {e}")
        except Exception as e:
    ...
    async def show_battle_results(self, battle: Battle, client: Client):
        ...
        try:
            ...
            if winner_id:
                ...
                base_winner_tokens = 50000
                base_winner_shards = 5000
                loser_penalty_tokens = 8000

                # Only award winner rewards if the defeated user's team had 5+ Pokemon
                if loser_team_size >= 5:
                    winner_reward_tokens = base_winner_tokens
                    requested_winner_shards = base_winner_shards
                else:
                    winner_reward_tokens = 0
                    requested_winner_shards = 0

                # Enforce a lifetime cap of 50,000 shards from battle rewards per user
                awarded_winner_shards = 0
                try:
                    pool = get_postgres_pool()
                    if pool:
                        async with pool.acquire() as conn:
                            ...
                            await conn.execute(
                                '''
                                CREATE TABLE IF NOT EXISTS battle_rewards_counters (
                                    user_id BIGINT PRIMARY KEY,
                                    shards_earned BIGINT NOT NULL DEFAULT 0,
                            ...
                            row = await conn.fetchrow(
                                'SELECT shards_earned FROM battle_rewards_counters WHERE user_id = $1',
                                winner_id,
                            )
                            current_shards_from_battles = int(row['shards_earned']) if row and 'shards_earned' in row else 0
                            remaining_cap = max(0, 50000 - current_shards_from_battles)
                            awarded_winner_shards = min(requested_winner_shards, remaining_cap)

                            # Give rewards (tokens always; shards only if remaining)
                            await self._give_rewards(winner_id, winner_reward_tokens, awarded_winner_shards)
                            await self._deduct_tokens(loser_id, loser_penalty_tokens)

                            if awarded_winner_shards > 0:
                                # Upsert the new counter value, clamp to cap
                                await conn.execute(
                                    '''
                                    INSERT INTO battle_rewards_counters (user_id, shards_earned, updated_at)
                                    VALUES ($1, LEAST(50000, $2), CURRENT_TIMESTAMP)
                                    ON CONFLICT (user_id) DO UPDATE SET
                                        shards_earned = LEAST(50000, battle_rewards_counters.shards_earned + EXCLUDED.shards_earned),
                                        updated_at = CURRENT_TIMESTAMP
                                    ''',
                                    winner_id,
                                    awarded_winner_shards,
                                )
                    else:
                        # No pool available; still try to give only tokens to avoid shard overflow without tracking
                ...
                if awarded_winner_shards > 0:
                    rewards_lines.append(
                        f"• <a href='tg://user?id={winner_id}'>{winner_name}</a> earned: +{winner_reward_tokens:,} tokens, +{awarded_winner_shards:,} shards"
                    )
                else:
...
async def battle_command(client: Client, message: Message):
    ...
    if result == "challenge_created":
        ...
        await message.reply_text(
            ...
            "You can battle the same opponent at most <b>15 times</b> per 24 hours.\n"
            "Try again later or battle a different opponent."
        )
