...
# Bot reaction function for successful Pokemon collection
async def add_collection_reaction(client: Client, message: Message):
    """Add a ⚡️ reaction to the user's collect command message when they successfully collect a Pokemon"""
    try:
        # Use Telegram Bot API directly for reactions
        url = f"https://api.telegram.org/bot{TOKEN}/setMessageReaction"
        
        # Correct data format for setMessageReaction
        data = {
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data) as response:
                ...
                if not result.get('ok'):
                    print(f"Reaction API error: {result.get('description', 'Unknown error')}")
                    
    except Exception as e:
        # Silently ignore reaction errors to avoid disrupting the collection process
        print(f"Could not add reaction to collection message: {e}")
        pass
...
async def handle_message(client: Client, message: Message):
    ...
    if group_id not in jackpot_next_interval:
        jackpot_next_interval[group_id] = random.randint(300, 400)
        ...
        if not message.command or len(message.command) == 1:
            ...
            if user_id in authorized_ids:
                ...
                try:
                    ...
                    # Add ⚡️ reaction to the user's collect command message
                    await add_collection_reaction(client, message)
...
async def handle_reply_collection(client: Client, message: Message):
    ...
    try:
        ...
        if not replied_message.from_user or replied_message.from_user.is_bot:
            ...
            if not character:
                # Not an active drop. Detect if the replied message is a drop-like message
                is_drop_like = False
                try:
                    rm = replied_message.reply_markup
                ...
                # Only show last-collected info if the user replied to a drop-like message
                if is_drop_like and hasattr(collect_command, "last_collected_drop") and collect_command.last_collected_drop.get(chat_id):
                    last_collected = collect_command.last_collected_drop[chat_id]
                    await message.reply_text(
                        f"<b>ℹ Last Character Was Already Collected By <a href=\"tg://user?id={last_collected['collected_by_id']}\">{last_collected['collected_by_name']}</a>!</b>",
                        disable_web_page_preview=True
                    )
                # Always stop processing here for non-active drops
                return
